...
namespace nPuzzle._Packs.LeanTween.Testing
{
	public class TestingRotate : MonoBehaviour {
		...
		void Start () {
			...
			Framework.LeanTween.rotateAround(earth, earth.transform.up, 360f, 5.0f).setPoint(sunLocalForEarth).setRepeat(-1);

			//method 2 leantween
			Vector3 sunLocalForEarth2 = earth2.transform.InverseTransformPoint(sun2.transform.position);
			Framework.LeanTween.rotateAroundLocal(earth2, earth2.transform.up, 360f, 5.0f).setPoint(sunLocalForEarth2);
