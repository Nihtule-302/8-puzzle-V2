...
namespace nPuzzle._Packs.LeanTween.Testing
{
	public class Testing240 : MonoBehaviour {
		...
		void Start () {
			Framework.LeanTween.moveY(cube1, cube1.transform.position.y - 15.0f, 10f).setEase(LeanTweenType.easeInQuad).setDestroyOnComplete(false).setOnComplete(()=>{
				Debug.Log("Done");
			});

			Vector3 before = cube1.transform.position;
			Framework.LeanTween.rotateAround(cube1, Vector3.forward, 360.0f, 10f).setOnComplete( ()=>{
...
//            Debug.Log("val:"+val);
			...
			Framework.LeanTween.value(gameObject, ScaleGroundColor, new Color(1f, 0f, 0f, 0.2f), Color.blue, 2f).setEaseInOutBounce();

			Framework.LeanTween.scale(cube2, Vector3.one * 2f, 1f).setEasePunch().setScale(5f);

			Framework.LeanTween.scale(rect1, Vector3.one * 2f, 1f).setEasePunch().setScale(-1f);
			...
			Framework.LeanTween.moveSplineLocal(sprite2,path,4f)
