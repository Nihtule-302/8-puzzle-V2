...
namespace nPuzzle._Packs.LeanTween.Testing
{
	public class TestingIssue2 : MonoBehaviour {
		...
		void Start () {
			descr = Framework.LeanTween.move(go, new Vector3(0f,0,100f), 10f);
...
//		LeanTween.scale(go2, Vector3.one * 4f, 10f).setEasePunch();
			...
			Framework.LeanTween.scaleX (go2, (go2.transform.localScale * 1.5f).x, 15f).setEase (LeanTweenType.punch);
			Framework.LeanTween.scaleY (go2, (go2.transform.localScale * 1.5f).y, 15f).setEase (LeanTweenType.punch);
			Framework.LeanTween.scaleZ (go2, (go2.transform.localScale * 1.5f).z, 15f).setEase (LeanTweenType.punch);
		}
		bool set = false;
		void Update () {
